                ...
                <div class="col-12">
                    <div class="card shadow-lg border-0 rounded-4">
                        <div class="card-header bg-dark text-white" style="background-color: #03436b !important;">
                            <h4 class="card-title mb-1"><i class="fas fa-plus-circle me-2"></i>{{ __('admin.products-create') ?? 'Create Product' }}</h4>
                            <p class="opacity-75 small mb-0">{{ __('admin.fill-details-to-create-product') ?? 'Fill in the details to create a new product' }}</p>
                        </div>
                        <div class="card-body p-4">
                            <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group mb-3">
                                            ...
                                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                                <option value="1" {{ old('status') == '1' ? 'selected' : '' }}>{{ __('admin.active') ?? 'Active' }}</option>
                                                <option value="0" {{ old('status') == '0' ? 'selected' : '' }}>{{ __('admin.inactive') ?? 'Inactive' }}</option>
                                            </select>
                                            @error('status')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                ...
                                <div class="d-flex justify-content-end mt-4 mb-2">
                                    <a href="{{ route('admin.products.index') }}" class="btn btn-outline-secondary me-2 px-4 py-2" style="min-width: 140px; font-size: 1rem; font-weight: bold; border-radius: 8px;">
                                    ...
                                    <button type="submit" class="btn text-white px-4 py-2" style="background-color: #03436b !important; min-width: 140px; font-size: 1rem; font-weight: bold; box-shadow: 0 4px 10px rgba(3, 67, 107, 0.3); border-radius: 8px;">
                                        <i class="fas fa-save me-2"></i> {{ __('admin.create') ?? 'Create' }}
                                    </button>
                                </div>
                            </form>
                        </div>
